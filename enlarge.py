...
patch_size = 96
margin = 12          # >= receptive-field radius (12 convs -> RF 25)

model = PlainCNN(12)
model.load_state_dict(torch.load('model_cnn.pt', map_location='cpu', weights_only=True)['model'])
model = model.to(device)
model.eval()

miku = Image.open('miku.png')
